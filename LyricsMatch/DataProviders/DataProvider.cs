    ...
    public static class DataProvider
    {
          ...
          try
          {
              ...
              if (rd.HasRows)
              {
                  while (rd.Read())
                  {
                      ...
                      song.Lyrics = ElasticDataProvider.GetLyricsByNameAuthor(song.Name, song.Author)[0].Lyrics;
                      songList.Add(song);
                  }
        ...
        public static List<Comment> GetSongComments(Song song)
        {
            ...
            try
            {
                conn.Open();
                MySqlCommand cmd = new MySqlCommand();
                cmd.CommandText = "SELECT * FROM comment WHERE song_id=" + song.Id + "";
                cmd.Connection = conn;
                MySqlDataReader rd = cmd.ExecuteReader();
                if (rd.HasRows)
                {
                    ...
                    while (rd.Read())
                    {
                        ...
                        comm.Song_id=Int32.Parse(rd["song_id"].ToString());
        ...
        public static List<String> DeleteComment(User loggedUser, string commentText,Song song)
        {
            ...
            try
            {
                conn.Open();
                MySqlCommand cmd = new MySqlCommand();
                cmd.CommandText = "DELETE FROM comment WHERE song_id=" + song.Id + " AND user_idd="+loggedUser.Id+" AND text=\""+commentText+"\"";
                cmd.Connection = conn;
                MySqlDataReader rd = cmd.ExecuteReader();
                rd.Close();
            }
        ...
        public static void AddComment(Song song, User user,String text)
        {
            ...
            try
            {
                ...
                cmd.CommandText = "INSERT INTO comment (text,song_id,user_idd) VALUES (\"" + text + "\"," + song.Id +"," + user.Id+")";
                cmd.Connection = conn;
                conn.Open();
